...
export function ServiceBands() {
  return (
    <section className="h-svh bg-[#ede1ee] px-1 pb-1 text-white">
      <div className="mx-auto grid h-full max-w-[1680px] grid-rows-3 overflow-hidden">
        {brand.serviceBands.map((service) => (
          <article
            key={service.title}
            className="group relative min-h-0 overflow-hidden border-t-[18px] border-[#ede1ee] first:border-t-0"
          >
            <Image
              src={service.image}
              ...
              unoptimized
            />
            <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(20, 23, 61,0.52),rgba(20, 23, 61,0.18)_36%,rgba(20, 23, 61,0.02)_72%)]" />
            <div className="relative z-10 flex h-full flex-col justify-center px-9 py-7 sm:px-16 lg:px-[11%]">
              ...
              >
                <ServiceGlyph />
                <span className="inline-flex h-14 items-center bg-[#f4ebf4]/92 px-7 text-[13px] font-semibold uppercase transition-colors group-hover:bg-white sm:h-16 sm:px-9">
                  {service.ctaLabel}
                </span>
              </Link>
            </div>
